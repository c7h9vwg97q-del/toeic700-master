const SRS = {
  ...
  getNextWord: (excludeId = null) => {
    ...
    let list = window.wordData.filter(w => 
      stats[w.id] && stats[w.id].dueAt <= now && w.id !== excludeId
    )
    if (list.length) return list[Math.floor(Math.random() * list.length)]

    list = window.wordData.filter(w => {
      const s = stats[w.id]
      return s && (s.correct + s.wrong > 0) &&
             (s.correct / (s.correct + s.wrong) < 0.7) &&
             w.id !== excludeId
    })
    if (list.length) return list[Math.floor(Math.random() * list.length)]

    list = window.wordData.filter(w => w.id !== excludeId)
